#!/usr/bin/env python3
"""
setup_cpu.py for RFX-Fuse - setup.py for RFX-Fuse - Breiman and Cutler's Unified ML & Similarity Engine

Original Random Forest algorithm by Leo Breiman and Adele Cutler
C++ implementation and Python bindings by Chris Kuchar
"""
...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        # Force CPU-only build for RFX-CPU package
        os.environ['RFX_CPU_ONLY'] = '1'
        ...
        cmake_args = [
            ...
            '-DRFX_CPU_ONLY=ON',  # Force CPU-only build
        ]
        
        print("=" * 60)
        print("Building RFX-CPU: CPU-only version (GPU disabled)")
        print("=" * 60)
...
long_description = ''
if os.path.exists('README_PYPI_CPU.md'):
    with open('README_PYPI_CPU.md', encoding='utf-8') as f:
