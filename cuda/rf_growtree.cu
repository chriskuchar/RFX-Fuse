...
__device__ void gpu_histogram_build_classification_device(
    ...
    for (integer_t i = 0; i < node_count; i++) {
        ...
        // Clamp bin to valid range (matches sparse behavior)
...
__device__ void gpu_histogram_find_split_classification_device(
    ...
    real_t& best_crit,          // Output: Fortran-style criterion (for MAXIMIZATION)
    real_t& best_split_value    // Output: split point value
) {
    // MATCHES GPU SPARSE rf_sparse_histogram.cu lines 950-1013 EXACTLY
    // Uses Fortran-style incremental criterion: (rln/rld) + (rrn/rrd)
    ...
    for (integer_t c = 0; c < nclass && c < 32; c++) {
        ...
        for (integer_t b = 0; b < n_bins; b++) {
            total_class[c] += hist[b * nclass + c];
        }
        total_weight += total_class[c];
    }

    // Initialize left/right sums (matches sparse exactly)
    real_t left_class[32];
    for (integer_t c = 0; c < nclass && c < 32; c++) left_class[c] = 0.0f;

    real_t rln = 0.0f, rld = 0.0f;  // Left: sum of squared counts, total weight
    real_t rrn = 0.0f, rrd = total_weight;  // Right: sum of squared counts, total weight

    // Initialize rrn = sum of total_class[c]^2
    for (integer_t c = 0; c < nclass && c < 32; c++) {
        rrn += total_class[c] * total_class[c];
    }

    real_t best_crit_local = -2.0f;  // Match sparse init
    integer_t best_bin = 0;

    // Scan bins for best split (matches sparse incremental formula)
    for (integer_t b = 0; b < n_bins - 1; b++) {
        // Update running sums incrementally (MATCHES SPARSE EXACTLY)
        for (integer_t c = 0; c < nclass && c < 32; c++) {
            real_t u = hist[b * nclass + c];
            if (u > 0.0f) {
                // Incremental update formulas from sparse:
                // rln += u * (u + 2 * left_class[c])
                // rrn += u * (u - 2 * (total_class[c] - left_class[c]))
        ...
        if (rld < 1.0f || rrd < 1.0f) continue;

        // Compute criterion (MATCHES SPARSE: (rln/rld) + (rrn/rrd))
    ...
    // Split at upper edge of best bin (MATCHES GPU SPARSE: edges[b + 1])
...
__device__ void gpu_histogram_build_regression_device(
    ...
    for (integer_t i = 0; i < node_count; i++) {
        ...
        // CRITICAL: Clamp bin to valid range (matches sparse behavior)
        if (bin >= n_bins) bin = n_bins - 1;
...
__device__ void gpu_histogram_build_classification_parallel(
    ...
    for (integer_t i = tid; i < node_count; i += num_threads) {
        ...
        // CRITICAL: Clamp bin to valid range (matches sparse behavior)
        // Don't skip samples - clamp them to the last bin
        if (bin >= n_bins) bin = n_bins - 1;
...
__device__ void gpu_histogram_find_split_classification_parallel(
    ...
    if (tid == 0) {
        ...
        best_crit = best_crit_local;
        // Split at upper edge of best bin (matches sparse)
        best_split_value = bin_edges[best_bin + 1];
    }
    __syncthreads();
}
...
__device__ void gpu_histogram_find_split_regression_parallel(
    ...
    if (tid == 0) {
        ...
        best_crit = best_mse;
        // FIX: Split point is at upper edge of best_bin (boundary between best_bin and best_bin+1)
        best_split_value = bin_edges[best_bin + 1];
    }
    __syncthreads();
}
...
__global__ void gpu_full_tree_kernel(
    ...
    for (integer_t kgrow = 0; kgrow < nnode[0] && iteration_count < max_iterations; ++kgrow, ++iteration_count) {
        ...
        if (node_sample_count < min_size_to_split) {
            ...
            // FIX: Must calculate and set nodeclass to majority class!
        ...
        real_t best_split_point = 0.0f;  // FIX: Store split point when found (node_samples gets overwritten per feature)
        integer_t jstat = 0;
        ...
        // FIX: Pre-generate random feature indices for this node using XorShift
        // XorShift is fast and avoids curand_init overhead per iteration
        integer_t node_mvars[32];  // Max mtry = 32
        {
            // Seed based on tree_id and kgrow for reproducibility
            uint32_t seed = static_cast<uint32_t>(tree_id) * 2654435761u +
                            static_cast<uint32_t>(kgrow) * 2246822519u + 12345u;
            for (integer_t mv = 0; mv < mtry && mv < 32; ++mv) {
                // XorShift32
                seed ^= seed << 13;
                seed ^= seed >> 17;
                seed ^= seed << 5;
                node_mvars[mv] = static_cast<integer_t>(seed % static_cast<uint32_t>(mdim));
            }
        }

    for (integer_t mv = 0; mv < mtry; ++mv) {
            // Use pre-generated random variable index
            integer_t mvar = (mv < 32) ? node_mvars[mv] : (mv % mdim);
            ...
            if (is_categorical && nclass > 1) {  // Only classification supports categoricals for now
                // Initialize curand state for categorical random sampling (large categoricals)
                curandState cat_local_state;
                curand_init(static_cast<unsigned long long>(tree_id) * 1000003ULL +
                            static_cast<unsigned long long>(kgrow) * 1000033ULL +
                            static_cast<unsigned long long>(mv) * 1000037ULL, 0, 0, &cat_local_state);
                ...
                if (nnz > 1) {
                    ...
                    if (numcat < ncmax) {
                        ...
                        gpu_catmaxr(tclasscat, tclasspop, pdo, numcat, nclass, igl,
                                   ncsplit, tmpclass, cat_critmax, igoleft, nhit, &cat_local_state);
                    }
            ...
            if (use_histogram && X_binned != nullptr && n_bins_per_feature != nullptr) {
                ...
                if (can_use_histogram) {
                    const real_t* bin_edges = bin_edges_all + mvar * (max_bins + 1);
                    ...
                    if (nclass > 1) {
                        ...
                        if (hist_best_crit > best_crit) {
                            ...
                            best_var = mvar;
                            best_split_point = hist_best_split;  // FIX: Also update best_split_point
                            if (threadIdx.x == 0) {
                    ...
                    if (sum_w_left > 1e-8f && sum_w_right > 1e-8f) {
                        ...
                        if (sum_w_left >= static_cast<real_t>(minndsize) &&
                            ...
                            if (crit < best_crit || (crit == best_crit && mvar < best_var)) {
                                ...
                                best_crit = crit;
                                best_var = mvar;
                                // FIX: Store split point NOW (node_samples gets overwritten for next feature)
                                integer_t s1 = node_samples[ii];
                                integer_t s2 = node_samples[ii + 1];
                                best_split_point = (x[s1 * mdim + mvar] + x[s2 * mdim + mvar]) / 2.0f;
                            }
                        }
                    }
                } else {
        ...
        if (threadIdx.x == 0) {
            ...
            if (nnode[0] + 2 <= maxnode) {
                ...
                // FIX: Use pre-computed best_split_point (stored when better split found)
                // Previously calculated from node_samples which was overwritten by last feature
                xbestsplit[kgrow] = best_split_point;
