...
__global__ void gpu_bin_dense_data_kernel(
    ...
    if (val >= edges[n_bins]) {
        X_binned[idx] = static_cast<uint8_t>(n_bins - 1);
        return;
    }
    
    // Binary search: find largest b where edges[b] <= val
    ...
    uint8_t bin = static_cast<uint8_t>(max(0, static_cast<int>(lo) - 1));
    if (bin >= n_bins) bin = static_cast<uint8_t>(n_bins - 1);
