...
__global__ void gpu_accumulate_oob_votes_kernel(
    ...
    integer_t n_samples,
    integer_t nclass,
    integer_t* oob_votes,             // Output: [nsample * nclass] vote counts
    ...
    for (integer_t n = tid; n < n_samples; n += stride) {
        ...
        if (nin[n] == 0) {
            ...
            if (pred_class >= 0 && pred_class < nclass) {
                ...
                ::atomicAdd(&oob_votes[n * nclass + pred_class], 1);
...
__global__ void gpu_adjust_casewise_votes_kernel(
    ...
    integer_t nclass,
    integer_t nnode,
    integer_t* oob_votes,             // [nsample * nclass] vote counts (modified in-place)
    ...
    for (integer_t n = tid; n < nsample; n += stride) {
        ...
        // Undo the +1 unweighted vote
        ::atomicSub(&oob_votes[n * nclass + pred_class], 1);
        ::atomicSub(&oob_counts[n], 1);
        
        // Add weighted vote (round to nearest integer, minimum 1)
        real_t vote_weight = tnodewt[kt];
        integer_t weighted_vote = static_cast<integer_t>(vote_weight + 0.5f);
        if (weighted_vote < 1) weighted_vote = 1;
        
        ::atomicAdd(&oob_votes[n * nclass + pred_class], weighted_vote);
        ::atomicAdd(&oob_counts[n], weighted_vote);
    }
}
