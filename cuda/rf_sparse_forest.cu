...
__global__ void gpu_compute_oob_predictions_kernel(
    const integer_t* oob_votes,   // [nsample * nclass] vote counts
    ...
    integer_t max_votes = oob_votes[tid * nclass];
...
integer_t train_sparse_forest_gpu(
    ...
    // ========================================================================
    integer_t* d_oob_votes;   // [nsample * nclass] - vote counts per class per sample
    integer_t* d_oob_counts;  // [nsample] - number of OOB predictions per sample
    cudaMalloc(&d_oob_votes, static_cast<size_t>(config.nsample) * static_cast<size_t>(config.nclass) * sizeof(integer_t));
    cudaMalloc(&d_oob_counts, static_cast<size_t>(config.nsample) * sizeof(integer_t));
    cudaMemset(d_oob_votes, 0, static_cast<size_t>(config.nsample) * static_cast<size_t>(config.nclass) * sizeof(integer_t));
    ...
    if (h_q != nullptr && h_nout != nullptr) {
        // Copy integer votes to temporary buffer, then convert to real_t
        std::vector<integer_t> h_oob_votes_int(config.nsample * config.nclass);
        cudaMemcpy(h_oob_votes_int.data(), d_oob_votes, 
                   config.nsample * config.nclass * sizeof(integer_t), cudaMemcpyDeviceToHost);
        
        // Convert to real_t for h_q
        for (integer_t i = 0; i < config.nsample * config.nclass; ++i) {
            h_q[i] = static_cast<real_t>(h_oob_votes_int[i]);
        }
