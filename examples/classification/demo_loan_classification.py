#!/usr/bin/env python3
"""
RFX Finance Pipeline Demo: Classification Systems

USE CASE 3: Classification Systems
Traditional Approach: XGBoost + SHAP + Isolation Forest (3 tools)
RFX Approach: One RFX model object

This demonstrates the RFX pipeline for explainable credit decisions:
...
RFX PROVIDES ALL 4 EXPLANATION TYPES:
  ...
  3. Overall Proximity Importance: "What clusters borrowers by risk?" (unique to RFX)
  4. Local Proximity Importance: "Why is this borrower similar to others?" (unique to RFX)

PIPELINE:
  Stage 1 — RFX Unsupervised: Borrower similarity + explanations (optional)
  Stage 2 — RFX Classifier: Default prediction + explanations

Comparisons included:
- RFX vs XGBoost: Classification accuracy, feature importance
...
def main():
    print("=" * 75)
    print("RFX FINANCE PIPELINE: Loan Denial Explanation")
    ...
    # =========================================================================
    # STAGE 1: RFX Unsupervised - COMMENTED OUT (Using classifier for all features)
    # =========================================================================
    # NOTE: For this finance use case, we use the classifier's proximity importance
    # and similarity methods instead of a separate unsupervised model.
    # The classifier provides all the same features (proximity importance, top-k similar,
    # outlier detection) while also providing predictions.
    unsup_time = 0.0  # No unsupervised model trained
    
    # =========================================================================
    # STAGE 2: RFX Classifier (Default Prediction)
    # =========================================================================
    print(f"\n{'='*75}")
    print("STAGE 2: RFX CLASSIFIER (Default Prediction / Loan Decision)")
    print("=" * 75)
    ...
    # Top-K Similar (from unsupervised - feature similarity)
    # Get more than TOP_K since unsupervised includes synthetic samples
    ...
    # Local Proximity Importance (from unsupervised)
    ...
    print("RFX Approach: One model object providing all capabilities")
    
    # Comparison 1: XGBoost (Classification)
    if HAS_XGBOOST:
        print(f"\nCOMPARISON 1: RFX vs XGBoost (Classification)")
    ...
    if HAS_SHAP and HAS_XGBOOST:
        print(f"\nCOMPARISON 2: RFX vs SHAP (Local Importance)")
...
both outcomes intuitively!"""
    ...
    print(f"\n{'='*75}")
    print("SUMMARY: RFX (1 Model) vs Traditional 3-Tool Approach")
    print("=" * 75)
    print(f"""
